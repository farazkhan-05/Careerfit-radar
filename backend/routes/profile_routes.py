...
from backend.models.api_schemas import CandidateProfileUpdate, PageResponse
...
def list_profiles(
    ...
    return paginate(
        db,
        select(db_models.CandidateProfile).order_by(
            db_models.CandidateProfile.created_at.desc()
        ),
        pagination,
    )
...
def update_profile(
    profile_id: UUID,
    payload: CandidateProfileUpdate = Body(...),
    db: Session = Depends(get_db),
) -> db_models.CandidateProfile:
...
def score_jobs(
    ...
    profile = db.execute(
        select(db_models.CandidateProfile)
    ...
    if profile is None:
        raise HTTPException(
            status_code=422,
            detail=(
                "No candidate profile found. Upload a resume with AI extraction "
                "enabled first."
            ),
        )
    ...
    unscored_jobs = select(db_models.Job).where(
        db_models.Job.id.not_in(scored_job_ids)
    )
...
def _requirements_for_scoring(
    ...
    if stored is not None:
        return _StoredReq(
            required_skills=_clean_requirement_list(
                stored.required_skills
            ),
